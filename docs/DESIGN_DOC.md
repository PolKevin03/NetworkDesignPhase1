...
Fill in the figures/plots required by the phase spec (if none, write “N/A”). N/A
...
- **New behaviors added:** UDP echo server, File Transfer using RDT 1.0
...
- Workstream A: UDP echo client/server (Kevin)
- Workstream B: File Transfer using RDT 1.0 (Kevin)
- Workstream C: Testing and recording (Kevin)
...
### 3.1 How to evolve the provided state diagram N/A
For each phase: 
...
- **Sender**
  - responsibilities: Read input message or file, send UDP packets to reciever, wait during file transfer
- **Receiver**
  - responsibilities: Listen to UDP port, echo messages, writes file data
- **Shared modules/utilities**
  - packet encode/decode: formats data and packets
  - checksum: N/A
  - logging/timing: Prints message
  - CLI/config parsing: read line arguments
...
|---|---:|---|---|---|
| type | 1 byte | int | packet type | 1=ACK and 0=Data |
| seq | 4 byte | int | sequence number | increments per packet |
| ack |  |  | ack number / flag |  | N/A
| len | 2 byte | int | payload length | last packet may be smaller |
| checksum |  |  | checksum value | what it covers (header/payload) | N/A
| payload | ≤ ~1024B | bytes | file chunk | binary-safe |

---

## 5) Data structures + module map N/A
...
## 7) Experiments + metrics plan (required if phase requires figures/plots) N/A not Phase 1
...
## 8) Edge cases + test plan N/A not Phase 1
...
|---|---|---|---|
| Packet format + encode/decode | Kevin | 01/26 | Packet showed and recored |
| Sender logic | Kevin | 01/26 | Sender sends and receives ACK |
| Receiver logic | Kevin | 01/26 | Reviever write and send ACK |
| Injection (if required) | N/A | N/A | Not Req. |
| Figures/plots (if required) | N/A | N/A | Not Req. |
| README + reproducibility | N/A | N/A | Not Req. |

### 10.2 Milestones (keep it realistic)
- Milestone 1: Submit phase 1 design document
- Milestone 2: Make UDP echo and file transfer
